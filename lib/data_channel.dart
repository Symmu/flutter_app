...
class _DataChannelSampleState extends State<DataChannelSample> {
  ...
  _open() async {
    ...
    try {
      ...
      print("offer:"+ offer.sdp.toString());
      //本地连接设置本地sdp信息
      ...
      print("answer:"+ answer.sdp.toString());
      //远端连接设置本地sdp信息
  ...
  _onLocalCandidate(RTCIceCandidate candidate) {
    print('LocalCandidate: ' + candidate.candidate.toString());
    //将本地Candidate添加至远端连接
  ...
  _onRemoteCandidate(RTCIceCandidate candidate) {
    print('RemoteCandidate: ' + candidate.candidate.toString());
    //将远端Candidate添加至本地连接
